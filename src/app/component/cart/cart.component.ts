      ...
      <div *ngIf="cartItems.length > 0" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-4">
          <div *ngFor="let item of cartItems" class="bg-white rounded-lg shadow-md p-4 flex items-center space-x-4">
            <img [src]="getItemImage(item)" [alt]="item.name" class="w-24 h-24 object-cover rounded" />
            <div class="flex-1">
              <h3 class="font-semibold text-lg">{{item.name}}</h3>
              <p class="text-gray-600">{{item.category}}</p>
              <p class="text-xl font-bold text-blue-600 mt-2">\${{item.price}}</p>
            </div>
            <button (click)="removeFromCart(item.id || item._id)" class="text-red-500 hover:text-red-700 transition">
...
export class CartComponent implements OnInit {
  ...
  removeFromCart(productId: number | string | undefined) {
    if (productId) {
      this.cartService.removeFromCart(Number(productId));
    }
  }

  getItemImage(item: Product): string {
    if (item.images && item.images.length > 0) {
      return item.images[0];
    }
    return item.image || 'https://images.unsplash.com/photo-1560393464-5c69a73c5770?w=400';
  }
}

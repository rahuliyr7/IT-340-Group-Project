...
@Component({
  ...
  template: `
    ...
    <!-- Products Grid -->
    <div *ngIf="!isLoading" class="max-w-7xl mx-auto px-4 pb-12">
      <div *ngIf="filteredProducts.length === 0" class="text-center py-12">
        <p class="text-xl text-gray-600">No products found</p>
      </div>

      <div *ngIf="filteredProducts.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div *ngFor="let product of filteredProducts" class="bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden">
          <div class="relative">
            ...
            <button (click)="toggleWatchlist(product)" 
              [class.bg-red-500]="isInWatchlist(product._id || product.id?.toString() || '')"
              [class.bg-white]="!isInWatchlist(product._id || product.id?.toString() || '')"
              class="absolute bottom-2 right-2 p-2 rounded-full shadow-lg hover:scale-110 transition">
              <svg class="w-5 h-5" 
                [attr.fill]="isInWatchlist(product._id || product.id?.toString() || '') ? 'white' : 'none'" 
                [attr.stroke]="isInWatchlist(product._id || product.id?.toString() || '') ? 'white' : 'red'" 
                viewBox="0 0 24 24">
          ...
          <div class="p-4">
            ...
            <div *ngIf="product.isAuction">
              <div class="flex items-center justify-between mb-2">
                <div>
                  ...
                  <p class="text-xl font-bold text-green-600">\${{product.currentBid || product.price}}</p>
                </div>
              ...
              <button (click)="placeBid(product)" class="w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-2 rounded-lg font-semibold hover:from-green-600 hover:to-green-700 transition">
                Place Bid
              </button>
            </div>
            ...
            <div *ngIf="!product.isAuction">
              ...
              <button (click)="purchaseProduct(product)" class="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-2 rounded-lg font-semibold hover:from-blue-600 hover:to-blue-700 transition">
                Buy Now
              </button>
            </div>
          </div>
...
export class HomeComponent implements OnInit {
  ...
  applyFilters() {
    ...
    if (this.selectedFilters.length > 0) {
      filters.category = this.selectedFilters[0];
    }
  ...
  placeBid(product: Product) {
    ...
    const bidAmount = prompt(`Current bid: $${product.currentBid || product.price}\nEnter your bid amount:`);
    
    if (bidAmount && !isNaN(Number(bidAmount))) {
      this.productService.placeBid(product._id!, Number(bidAmount)).subscribe({
        next: (response) => {
  ...
  getProductImage(product: Product): string {
    ...
    return product.image || 'https://images.unsplash.com/photo-1560393464-5c69a73c5770?w=400';
  }
}

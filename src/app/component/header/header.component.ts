    ...
    <div class="max-w-7xl mx-auto px-4 py-4">
      <div class="flex items-center justify-between">
        ...
        <div class="hidden lg:flex flex-1 max-w-2xl mx-8">
          <form (submit)="performSearch(); $event.preventDefault()" class="w-full">
            <div class="relative w-full">
              <input
                ...
                class="w-full px-4 py-3 pr-12 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-300"
              />
              <button 
                type="submit"
                class="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-md transition cursor-pointer z-10"
        ...
        <form (submit)="performSearch(); $event.preventDefault()" class="w-full">
          <div class="relative">
            <input
              ...
              name="searchMobile"
              placeholder="Search..."
              class="w-full px-4 py-2 pr-12 rounded-lg text-gray-900 focus:outline-none"
            />
            <button 
              type="submit"
              class="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-md transition cursor-pointer z-10"
              style="pointer-events: auto;">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
              </svg>
  ...
  onLogout() {
   ...
   performSearch() {
    if (this.searchQuery && this.searchQuery.trim()) {
      console.log('Header: Performing search for:', this.searchQuery);
      ...
      console.log('Header: Empty search query');
    }
  }
}
